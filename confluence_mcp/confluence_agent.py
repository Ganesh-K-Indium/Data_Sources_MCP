    ...
    async with streamablehttp_client(MCP_HTTP_STREAM_URL) as (read_stream, write_stream, _):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            agent = create_react_agent(
                model=model,
                tools=tools,
                name="ConfluenceOperationsAgent",
            ...
            print("="*80)
            print("\n📋 Example commands:")
            print("  • Upload file: 'Upload /path/to/document.pdf to page Documentation in space TEAM'")
            print("  • Search content: 'Find all pages about project planning in space TEAM'")
            print("  • List attachments: 'Show all PDF attachments in space DOCS'")
            print("  • Space stats: 'Generate statistics for space TEAM'")
            print("  • Find page: 'Find page titled Meeting Notes in space PROJECT'")
            print("\n💡 You can also specify company names for metadata when uploading PDFs")
            print("💡 Note: I can upload to existing pages but cannot create new pages")
