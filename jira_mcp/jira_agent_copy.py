...
async def main():
    ...
    async with streamablehttp_client(MCP_HTTP_STREAM_URL) as (read_stream, write_stream, _):
        async with ClientSession(read_stream, write_stream) as session:
            ...
            print("  • Create issue: 'Create a new task for quarterly review in project FINANCE'")
            print("  • Create and upload: 'Create issue for Amazon 10K filing and upload the PDF'")
            ...
            print("\n💡 NEW: I can now create issues automatically when uploading to non-existent issues!")
            print("💡 PDFs are automatically processed for vector search when uploaded")
            print("💡 Smart workflow: Upload -> Create issue if needed -> Process for search")
